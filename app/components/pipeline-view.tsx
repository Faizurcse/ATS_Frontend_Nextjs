...
export default function PipelineView() {
  ...
  const [candidates, setCandidates] = useState<PipelineCandidate[]>([
    ...
    ...Array.from({ length: 25 }, (_, i) => ({
      ...
      appliedDate: `2024-01-${String(Math.floor(Math.random() * 20) + 1).padStart(2, "0")}`,
      lastUpdated: `2024-01-${String(Math.floor(Math.random() * 20) + 1).padStart(2, "0")}`,
      ...
      lastActivity: `2024-01-${String(Math.floor(Math.random() * 20) + 1).padStart(2, "0")}T10:00:00Z`,
      aiScore: 60 + Math.floor(Math.random() * 40),
      aiVerdict: ["highly_recommended", "recommended", "consider", "not_recommended"][i % 4] as any,
      profileStrength: 60 + Math.floor(Math.random() * 40),
      responseRate: 70 + Math.floor(Math.random() * 30),
      engagementScore: 65 + Math.floor(Math.random() * 35),
    })),
  ])
